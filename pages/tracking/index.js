...
// AGGIUNGI QUI LA NUOVA FUNZIONE setupBulkActions
window.setupBulkActions = function() {
    ...
    bulkActions.className = 'bulk-actions-container';
...
window.trackingInit = async function() {
    ...
    try {
        ...
        if (window.ApiStatusIndicator) {
            await window.ApiStatusIndicator.init();
            console.log('✅ [Tracking] API status indicator initialized');
        }
        ...
        // Esponi getColumnFormatter PRIMA di setupTrackingTable
       ...
       // Setup page components - ORDINE IMPORTANTE!
        ...
        // NUOVO: Listen for API keys updates
        window.addEventListener('apiKeysUpdated', async (event) => {
            console.log('[Tracking] API keys updated, reinitializing service...');
            if (window.trackingService) {
                await window.trackingService.initialize();
            }
        });
...
async function handleAddTracking(event) {
    ...
    // NUOVO: Controlla prima se le API sono configurate
    if (window.trackingService && !window.trackingService.hasApiKeys()) {
        const proceed = await window.ModalSystem.confirm({
            title: 'API Non Configurate',
            message: 'Le API ShipsGo non sono configurate. Il tracking funzionerà in modalità demo. Vuoi continuare?',
            confirmText: 'Continua in Demo',
            cancelText: 'Configura API'
        });
    ...
    try {
        ...
        if (trackingResult.success) {
            ...
            // MODIFICATO: Salva su Supabase se disponibile, altrimenti localStorage
            if (window.supabaseTrackingService) {
                try {
                    const savedTracking = await window.supabaseTrackingService.createTracking(enrichedData);
                    if (savedTracking) {
                        console.log('✅ [Tracking] Salvato su Supabase:', savedTracking.id);
                        
                        // Ricarica la lista completa da Supabase
                        await loadTrackings();
                        ...
                        // Notifica successo
                        notificationSystem.success('Tracking salvato su Supabase!');
                    }
                } catch (error) {
                    console.error('❌ [Tracking] Errore Supabase:', error);
                    ...
                    notificationSystem.dismiss('tracking-loading');
                    notificationSystem.warning('Salvato localmente (Supabase non disponibile)');
                }
            } else {
                // Fallback to localStorage
                trackings.push(enrichedData);
                ...
                if (trackingResult.mockData) {
                    notificationSystem.info('Tracking aggiunto localmente (modalità demo)');
                } else if (trackingResult.fromCache) {
                    notificationSystem.success('Tracking aggiunto localmente (da cache)');
                } else {
                    notificationSystem.success('Tracking aggiunto localmente con dati live!');
                }
            }
        ...
        // Prova comunque a salvare
        if (window.supabaseTrackingService) {
            try {
                await window.supabaseTrackingService.createTracking(formData);
                await loadTrackings();
                notificationSystem.warning('Tracking aggiunto manualmente su Supabase');
            } catch (supabaseError) {
                trackings.push(formData);
                localStorage.setItem('trackings', JSON.stringify(trackings));
                await loadTrackings();
                notificationSystem.warning('Tracking aggiunto manualmente (locale)');
            }
        } else {
            trackings.push(formData);
            localStorage.setItem('trackings', JSON.stringify(trackings));
        ...
        if (refreshResult.success) {
            ...
            if (window.supabaseTrackingService) {
                try {
                    ...
                    console.log('✅ [Tracking] Aggiornato su Supabase');
                } catch (error) {
...
// Handle delete tracking - MODIFICATA PER SUPABASE
async function handleDeleteTracking(id) {
    ...
    try {
        ...
        // MODIFICATO: Elimina da Supabase se disponibile
        if (window.supabaseTrackingService) {
            try {
                await window.supabaseTrackingService.deleteTracking(id);
                console.log('✅ [Delete] Eliminato da Supabase');
                
                // Ricarica la lista
                await loadTrackings();
                
                // Show success notification
                notificationSystem.success('Tracking eliminato da Supabase');
            } catch (error) {
            ...
            // Show success notification
            notificationSystem.success('Tracking eliminato localmente');
        }
