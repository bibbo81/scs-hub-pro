...
carriers.sort((a, b) => {
   ...
   function getCarriersByType(type) {
       const carriers = {
           ...
           awb: [
            { iata: '176', name: 'EMIRATES', display: 'Emirates (176)' },
            { iata: '999', name: 'AIR CHINA', display: 'Air China (999)' },
    { iata: '074', name: 'AIR FRANCE CARGO', display: 'Air France (074)' },
    { iata: '057', name: 'AIR FRANCE CARGO', display: 'Air France (057)' },
    { iata: '129', name: 'AIR FRANCE CARGO', display: 'Air France (129)' },
    { iata: '139', name: 'AEROMEXICO CARGO', display: 'Aeromexico (139)' },
    { iata: '390', name: 'AEGEAN AIRLINES', display: 'Aegean Airlines (390)' },
    { iata: '053', name: 'AER LINGUS', display: 'Aer Lingus (053)' },
    { iata: '543', name: 'AERCARIBE', display: 'AerCaribe (543)' },
    { iata: '497', name: 'AERCARIBE', display: 'AerCaribe (497)' },
    { iata: '555', name: 'AEROFLOT', display: 'Aeroflot (555)' },
    { iata: '044', name: 'AEROLINEAS ARGENTINAS', display: 'Aerolineas (044)' },
    { iata: '873', name: 'AEROUNION', display: 'Aerounion (873)' },
    { iata: '277', name: 'AFRIJET', display: 'Afrijet (277)' },
    { iata: '124', name: 'AIR ALGERIE', display: 'Air Algerie (124)' },
    { iata: '514', name: 'AIR ARABIA', display: 'Air Arabia (514)' },
    { iata: '807', name: 'AIR ASIA', display: 'Air Asia (807)' },
    { iata: '900', name: 'AIR ASIA', display: 'Air Asia (900)' },
    { iata: '843', name: 'AIR ASIA', display: 'Air Asia (843)' },
    { iata: '657', name: 'AIR BALTIC', display: 'Air Baltic (657)' },
    { iata: '142', name: 'AIR BELGIUM', display: 'Air Belgium (142)' },
    { iata: '014', name: 'AIR CANADA', display: 'Air Canada (014)' },
    { iata: '996', name: 'AIR EUROPA', display: 'Air Europa (996)' },
    { iata: '631', name: 'AIR GREENLAND', display: 'Air Greenland (631)' },
    { iata: '994', name: 'AIR INCHEON', display: 'Air Incheon (994)' },
    { iata: '098', name: 'AIR INDIA', display: 'Air India (098)' },
    { iata: '675', name: 'AIR MACAU CARGO', display: 'Air Macau (675)' },
    { iata: '643', name: 'AIR MALTA', display: 'Air Malta (643)' },
    { iata: '239', name: 'AIR MAURITIUS', display: 'Air Mauritius (239)' },
    { iata: '086', name: 'AIR NEW ZEALAND', display: 'Air New Zealand (086)' }
],
           bl: [
               { code: 'MSK', name: 'Maersk Line' },
               { code: 'MSC', name: 'Mediterranean Shipping Company' },
   ...
   if (departureEvent) {
       ...
       console.log('📅 Departure date trovata:', departureDate);
                       ...
                       const mappedData = {
                           ...
                           _raw_api_response: apiResponse.mappedFields?._raw_api_response || apiResponse._raw_api_response || {}
                       };
           ...
           ...(formData.trackingType === 'awb' ? {
            ...
            destination_port: formData._raw_api_response?.route?.destination?.location?.iata || formData.destination || '-',
               ...
               date_of_departure: (() => {
                   ...
                   // Fallback: usa date_of_dep dal route
               ...
               // CONTAINER COUNT (PIECES) - LEGGI DAL RAW
               ...
               created_at_shipsgo: (() => {
                   ...
                   if (createdAt) {
                       // Formato: "2025-05-21 09:46:47" -> "21/05/2025"
                       return createdAt.split(' ')[0].split('-').reverse().join('/');
                   }
                   return formatDateDDMMYYYY(new Date().toISOString());
               })(),
               
               // IMPORTANTE: Campo departure per la tabella (copia di date_of_departure)
               departure: (() => {
                   const movements = formData._raw_api_response?.movements || [];
                   const depMovement = movements.find(m => m.event === 'DEP');
                   if (depMovement && depMovement.timestamp) {
               ...
               // Altri campi necessari dalla formData originale
               origin: formData.origin || formData._raw_api_response?.route?.origin?.location?.iata || '-',
               destination: formData.destination || formData._raw_api_response?.route?.destination?.location?.iata || '-',
...
vessel_name: formData.vessel_name || apiResponse.metadata?.raw?.shipment?.containers?.[0]?.movements?.slice(-1)[0]?.vessel?.name || '-',
vessel_imo: formData.vessel_imo || apiResponse.metadata?.raw?.shipment?.containers?.[0]?.movements?.slice(-1)[0]?.vessel?.imo || '-',
voyage_number: formData.voyage_number || apiResponse.metadata?.raw?.shipment?.containers?.[0]?.movements?.slice(-1)[0]?.voyage || '-',
container_size: formData.container_size || apiResponse.metadata?.raw?.shipment?.containers?.[0]?.size || '-',
