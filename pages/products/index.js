...
showStatus(message, type = 'info', duration = 3000) {
 ...
 showImportModal = async () => {
  ...
  renderProductsList() {
    ...
    const drag = '<span class="column-header-handle drag-handle" onmousedown="event.stopPropagation();"><i class="fas fa-grip-vertical"></i></span>';
    const headerCells = this.visibleColumns.map(key => {
      const col = this.allColumns.find(c => c.key === key) || { label: this.toLabel(key) };
      const sort = (k, label) => `${drag}<span class="sortable" onclick="window.productIntelligenceSystem.sortProducts('${k}')">${label} ${this.getSortIcon(k)}</span>`;
      switch (key) {
        case 'sku':
          return `<th data-key="${key}">${sort('sku', 'SKU')}</th>`;
        case 'name':
          return `<th data-key="${key}">${sort('name', 'Product Name')}</th>`;
        case 'category':
          return `<th data-key="${key}">${sort('category', 'Category')}</th>`;
        case 'shippingCost':
          return `<th data-key="${key}">${sort('shippingCost', 'Avg Shipping Cost')}</th>`;
        case 'costTrend':
          return `<th data-key="${key}">${sort('costTrend', 'Cost Trend')}</th>`;
        case 'unitsShipped':
          return `<th data-key="${key}">${sort('unitsShipped', 'Units Shipped')}</th>`;
        case 'profitImpact':
          return `<th data-key="${key}">${sort('profitImpact', 'Profit Impact')}</th>`;
        case 'status':
          return `<th data-key="${key}">${drag}<span>Status</span></th>`;
        case 'actions':
          return `<th data-key="${key}">${drag}<span>Actions</span></th>`;
        default:
          return `<th data-key="${key}">${drag}<span>${col.label}</span></th>`;
      }
    }).join('');
    ...
    this.products.forEach(p => {
      Object.keys(p).forEach(k => {
        if (!existing.has(k) && !exclude.includes(k)) {
          ...
          if (!this.visibleColumns.includes(k)) this.visibleColumns.push(k);
        }
      });
    });
  }
