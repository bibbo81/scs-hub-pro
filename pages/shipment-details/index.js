...
function formatStatus(rawStatus) {    const statusKey = (rawStatus || 'registered').toLowerCase().replace(/ /g, '_');    const label = rawStatus || 'Registrato';    return `<span class="status-badge status-${statusKey}">${label}</span>`;}
...
async function deleteAdditionalCost(costId) {
    ...
    if (confirmed) {
        try {
            notificationSystem.info('Eliminazione del costo in corso...');
            await dataManager.deleteAdditionalCost(costId);
            notificationSystem.success('Costo eliminato con successo!');
            loadShipmentDetails(getShipmentIdFromURL());
        } catch (error) {
