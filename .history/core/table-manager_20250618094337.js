...
export class TableManager {
    ...
    // Aggiungi classe durante il reordering
    table.classList.add('dragging');
    
    // Funzione helper per muovere elementi
    const moveElement = (row, fromIndex, toIndex) => {
        const cells = Array.from(row.children);
        const element = cells[fromIndex];
        if (!element) return;
        
        if (toIndex >= cells.length) {
            row.appendChild(element);
        } else if (fromIndex < toIndex) {
            row.insertBefore(element, cells[toIndex + 1] || null);
        } else {
            row.insertBefore(element, cells[toIndex]);
        }
    };
    ...
    if (headerRow) {
        moveElement(headerRow, oldIndex, newIndex);
    }
    
    // Muovi tutte le celle del body
    const bodyRows = table.querySelectorAll('tbody tr');
    bodyRows.forEach(row => {
        moveElement(row, oldIndex, newIndex);
    });
    
    // Forza il browser a ridisegnare
    void table.offsetHeight;
    
    // Rimuovi classe dopo il reordering
    setTimeout(() => {
