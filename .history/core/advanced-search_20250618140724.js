...
export class AdvancedSearch {
    ...
    getStatusMapping() {
        return {
            ...
            'LA spedizione è stata consegnata': 'consegnato', // Variante maiuscola
            ...
            'la spedizione e stata creata dal mittente, attendiamo che ci venga affidata per l invio a destinazione.': 'spedizione creata', // Variante senza apostrofi
            'la spedizione è stata consegnata': 'consegnato',
            ...
            'arrivata': ['arrived'],
            'arrivato': ['arrived'], // Supporto maschile
            'consegnato': ['delivered', 'la spedizione è stata consegnata', 'LA spedizione è stata consegnata', 'consegnata.'],
            ...
            'sdoganato': ['international shipment release - import'], // Supporto maschile
            'spedizione creata': ['shipment information sent to fedex', 'la spedizione e\' stata creata dal mittente, attendiamo che ci venga affidata per l\'invio a destinazione.', 'la spedizione e stata creata dal mittente, attendiamo che ci venga affidata per l invio a destinazione.'],
    ...
    // Mappa per i tipi di tracking
    getTypeMapping() {
        return {
            // Italiano -> Tipi sistema
            'mare': ['container', 'bl'],
            'aereo': ['awb'],
            'aria': ['awb'],
            'parcel': ['parcel'],
            ...
            // Inglese -> Tipi sistema
            'sea': ['container', 'bl'],
            'air': ['awb'],
            'ocean': ['container', 'bl'],
            'maritime': ['container', 'bl'],
            'flight': ['awb'],
    ...
    searchWithTypeMapping(row, searchTerm) {
        ...
        // Se il tipo della riga ha un mapping italiano
    ...
    globalSearch(data, query) {
        ...
        return data.filter(row => {
            ...
            // Ricerca con mapping tipi per termini come "mare", "aereo"
            if (this.searchWithTypeMapping(row, searchTerm)) {
                return true;
            }
    ...
    searchWithStatusMapping(row, searchTerm) {
        ...
        // Controllo case-insensitive per varianti maiuscole/minuscole
        for (const [key, value] of Object.entries(statusMap)) {
            if (key.toLowerCase() === rowStatus && 
                (value === searchTerm || value.toLowerCase() === searchTerm)) {
                return true;
            }
            
            if (Array.isArray(value) && value.some(v => v.toLowerCase() === rowStatus) && 
