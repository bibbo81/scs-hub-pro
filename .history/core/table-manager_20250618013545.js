...
export class TableManager {
    ...
    renderControls() {
        const suggestions = this.searchTerm ? 
            this.advancedSearch.getSuggestions(this.searchTerm, this.data, this.getColumns()) : 
            [];
                ...
                ${this.options.searchable ? `
                    <div class="sol-table-search">
                        ...
                        <input 
                            ...
                            placeholder="Cerca... (usa campo:valore per ricerca specifica)"
                            value="${this.searchTerm}"
                            autocomplete="off"
                        >
                        ${this.searchTerm ? `
                            <button class="search-clear" onclick="this.previousElementSibling.value=''; this.previousElementSibling.dispatchEvent(new Event('input'));">
                            ...
                            <div class="search-suggestions">
                                ${suggestions.map(s => `
                                    <div class="suggestion-item" data-value="${s.value}">
                                        <i class="fas fa-${s.type === 'history' ? 'history' : s.type === 'field' ? 'filter' : 'search'}"></i>
                    ...
                    <div class="search-info">
                        ${this.searchTerm ? `${this.filteredData.length} risultati trovati` : ''}
                    </div>
                ` : ''}
    ...
    applyFilters() {
        ...
        if (this.searchTerm) {
            ...
            console.log(`Found ${filtered.length} results for: ${this.searchTerm}`);
        }
        ...
        suggestionItems.forEach(item => {
            item.addEventListener('click', () => {
                ...
                if (searchInput) {
                    ...
                    this.advancedSearch.addToHistory(value);
                }
            });
        });
    ...
    handleColumnReorder(oldIndex, newIndex) {
        ...
        // IMPORTANTE: Aggiorna SUBITO il DOM invece di aspettare il re-render
    ...
    reorderTableColumns(oldIndex, newIndex) {
        ...
        if (headerRow) {
            ...
            if (headers[oldIndex] && headers[newIndex]) {
                const headerToMove = headers[oldIndex];
                const referenceHeader = oldIndex < newIndex ? 
                    (headers[newIndex].nextSibling || null) : 
                    headers[newIndex];
        ...
        // Gestisci tutte le righe del body
        const bodyRows = table.querySelectorAll('tbody tr');
        bodyRows.forEach(row => {
            const cells = row.querySelectorAll('td');
            if (cells[oldIndex] && cells[newIndex]) {
                const cellToMove = cells[oldIndex];
                const referenceCell = oldIndex < newIndex ? 
                    (cells[newIndex].nextSibling || null) : 
                    cells[newIndex];
