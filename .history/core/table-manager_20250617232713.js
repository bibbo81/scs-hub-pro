...
renderTable() {
    ...
    renderHeader(column) {
        ...
        return `
            <th class="${isSortable ? 'sol-table-sortable' : ''}" 
...
enableColumnDrag() {
    ...
    if (!headerRow || !window.Sortable) return;
    ...
    this.columnSortable = new Sortable(headerRow, {
        ...
        handle: 'th', // Permette di trascinare cliccando ovunque sull'header
...
handleColumnReorder(oldIndex, newIndex) {
    ...
    const movedColumn = this.config.columns.splice(oldIndex, 1)[0];
    this.config.columns.splice(newIndex, 0, movedColumn);
...
saveColumnOrder() {
    const columnOrder = this.config.columns.map(col => col.key);
...
// Modifica il metodo init per includere il drag & drop
init() {
...
/* Indicatore visivo di trascinabilità */
...
.column-drag-chosen {
    background: var(--sol-gray-100) !important;
...
/* Animazione smooth per il riordino */
.data-table thead tr {
...
.data-table th.sortable-chosen + th {
    border-left: 3px solid var(--sol-primary);
}

/* Mobile - disabilita drag su schermi piccoli */
