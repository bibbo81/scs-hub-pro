...
export class AdvancedSearch {
    ...
    globalSearch(data, query) {
        ...
        return data.filter(row => {
            ...
            return Object.entries(row).some(([key, value]) => {
                ...
                // Ricerca esatta nel valore originale
                if (stringValue.includes(searchTerm)) {
                    return true;
                }
                
                // Se esiste una mappatura per questo campo, cerca anche nel valore mappato
                const mappedValue = this.getMappedValue(key, value);
                if (mappedValue && mappedValue.toLowerCase().includes(searchTerm)) {
                    return true;
                }
                
                // Ricerca fuzzy per correggere errori di battitura
                return this.fuzzyMatch(stringValue, searchTerm) || 
                       (mappedValue && this.fuzzyMatch(mappedValue.toLowerCase(), searchTerm));
            });
        });
    }
    ...
    matchesValue(value, searchTerm) {
        ...
        // Ricerca fuzzy nel valore originale
        if (this.fuzzyMatch(stringValue, term)) {
            return true;
        }
    ...
    // Ottieni il valore mappato se esiste una mappatura
    getMappedValue(fieldKey, originalValue) {
        // Controlla se esiste una mappatura globale per status
        if (fieldKey === 'status' && window.statusMapping) {
            return window.statusMapping[originalValue] || null;
        }
        
        // Controlla se esiste una mappatura per altri campi
        const mappingKey = `${fieldKey}Mapping`;
        if (window[mappingKey]) {
            return window[mappingKey][originalValue] || null;
        }
    ...
    getFieldValueSuggestions(fieldName, partialValue, data, columns) {
        ...
        data.forEach(row => {
            const originalValue = this.getCellValue(row, column.key);
            if (originalValue !== null && originalValue !== undefined) {
                const stringValue = String(originalValue);
                
                // Aggiungi il valore originale
                if (!partialValue || stringValue.toLowerCase().includes(partialValue.toLowerCase())) {
                    uniqueValues.add(stringValue);
                }
                
                // Aggiungi il valore mappato se esiste
                const mappedValue = this.getMappedValue(column.key, originalValue);
                if (mappedValue && (!partialValue || mappedValue.toLowerCase().includes(partialValue.toLowerCase()))) {
