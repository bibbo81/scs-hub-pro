...
class TrackingService {
    ...
    async addContainerToShipsGo(containerNumber) {
        const url = `${this.apiConfig.v1.baseUrl}/container/add?authCode=${this.apiConfig.v1.authCode}`;
        ...
        if (!response.ok) {
            ...
            throw new Error(data.message || 'Failed to add container to ShipsGo');
        }
    ...
    async getContainerInfo(containerNumber) {
        const url = `${this.apiConfig.v1.baseUrl}/container/info?authCode=${this.apiConfig.v1.authCode}&containerNumber=${containerNumber.toUpperCase()}`;
        ...
        if (!response.ok) {
            ...
            throw new Error(errorData.message || 'Failed to get container info');
        }
    ...
    async addAWBToShipsGo(awbNumber) {
        const url = `${this.apiConfig.v2.baseUrl}/airtracking/shipments`;
        
        const response = await fetch(url, {
            method: 'POST',
            headers: {
                'X-Shipsgo-User-Token': this.apiConfig.v2.userToken,
        ...
        if (!response.ok) {
            ...
            throw new Error(data.message || 'Failed to add AWB to ShipsGo');
        }
    ...
    async getAWBInfo(awbNumber) {
        const url = `${this.apiConfig.v2.baseUrl}/airtracking/shipments?awbNumber=${awbNumber.toUpperCase()}`;
        ...
        if (!response.ok) {
            const errorData = await response.json();
            throw new Error(errorData.message || 'Failed to get AWB info');
        }
    ...
    async testConnection() {
        ...
        if (this.apiConfig.v1?.enabled) {
            try {
                // Test semplice per ShipsGo v1.2
                const testUrl = `${this.apiConfig.v1.baseUrl}/test?authCode=${this.apiConfig.v1.authCode}`;
        ...
        if (this.apiConfig.v2?.enabled) {
            try {
                // Test per ShipsGo v2.0
                const testUrl = `${this.apiConfig.v2.baseUrl}/test`;
