// ===== MODAL SYSTEM DEFINITIVO - SOLUZIONE COMPLETA AI MODAL SOVRAPPOSTI =====
// Questo file SOSTITUISCE completamente modal-system.js e rimuove tutti i conflitti

(function() {
    'use strict';
    
    console.log('🔧 Loading DEFINITIVE Modal System - Anti-Overlap Solution');
    
    // ===== FORCE CLEANUP ALL EXISTING MODAL SYSTEMS =====
    function forceCleanupExistingModals() {
        // Remove ALL modal overlays from DOM
        ...
        // Clear any existing modal references
    ...
    // ===== DEFINITIVE MODAL SYSTEM CLASS =====
    class DefinitiveModalSystem {
        constructor() {
            this.activeModals = new Map();
            this.confirmResolvers = new Map();
            this.zIndexCounter = 3000; // Higher z-index to override conflicts
            this.isInitialized = false;
            this.preventDoubleClick = new Set(); // Prevent double-click issues
        ...
        init() {
            ...
            console.log('✅ Definitive Modal System initialized');
        }

        setupStyles() {
            // Remove existing styles first
            document.querySelectorAll('#unified-modal-styles, #modal-styles, #definitive-modal-styles').forEach(el => {
                el.remove();
            });
            
            const style = document.createElement('style');
            style.id = 'definitive-modal-styles';
            style.textContent = `
                .sol-modal-overlay {
        ...
        setupGlobalHandlers() {
            ...
            // Global click handler with anti-double-click protection
            this.globalClickHandler = (e) => {
                ...
                // Handle OK/Close/Cancel buttons by text
                const text = target.textContent?.trim().toLowerCase();
                if (['ok', 'okay', 'chiudi', 'close', 'annulla', 'cancel'].includes(text)) {
                    e.preventDefault();
                    e.stopPropagation();
                    
                    const modal = target.closest('.sol-modal-overlay');
                    if (modal) {
                        // Special handling for confirm dialogs
            ...
            document.addEventListener('click', this.globalClickHandler, true); // Use capture phase
        }

        show(options = {}) {
            // Force cleanup before showing new modal
        ...
        confirm(options = {}) {
            return new Promise((resolve) => {
                ...
                const modal = this.show({
                    ...
                    onClose: () => {
                        // Auto-resolve with false if modal closed without choice
        ...
        progress(options = {}) {
            ...
            return {
                ...
                setStats: (stats) => {
                    // Compatibility method for ImportManager
        ...
        alert(options = {}) {
            ...
            return this.confirm({
                ...
                cancelText: null // Solo un bottone
            });
        }
        ...
        close(modalId) {
            ...
            if (!modalData) {
                // Try to find modal in DOM if not in our map
    ...
    // ===== INITIALIZATION =====
    function initializeDefinitiveModalSystem() {
        console.log('🔧 Initializing DEFINITIVE Modal System...');
        
        // Force cleanup any existing systems
        ...
        // Create new definitive instance
        const definitiveModalSystem = new DefinitiveModalSystem();
        
        // Assign to all global variants
        window.ModalSystem = definitiveModalSystem;
        window.modalSystem = definitiveModalSystem;
        
        // Add debug helpers
        window.debugModals = () => {
            console.log('🐛 Modal System Debug:', definitiveModalSystem.debugInfo());
        };
        
        window.forceCloseAllModals = () => {
            definitiveModalSystem.closeAll();
            console.log('🧹 Force closed all modals');
        };
        
        console.log('✅ DEFINITIVE Modal System ready');
        console.log('📊 Debug commands: window.debugModals(), window.forceCloseAllModals()');
        
        return definitiveModalSystem;
    }

    // ===== AUTO-INITIALIZATION =====
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', () => {
            setTimeout(initializeDefinitiveModalSystem, 100);
        });
    } else {
        setTimeout(initializeDefinitiveModalSystem, 100);
    }
    ...
    if (typeof module !== 'undefined' && module.exports) {
        module.exports = DefinitiveModalSystem;
    }
    
    if (typeof window !== 'undefined' && window.define && window.define.amd) {
        window.define([], () => DefinitiveModalSystem);
    }

    console.log('🎯 DEFINITIVE Modal System loaded - Anti-Overlap Protection Active');
...
if (typeof window !== 'undefined' && window.ModalSystem) {
    // Export the initialized modal system
