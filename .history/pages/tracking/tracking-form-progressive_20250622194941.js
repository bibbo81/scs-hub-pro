...
...(formData.trackingType === 'awb' ? {
    ...
    carrier: formData.carrier || apiResponse?.carrier?.code || 'UNKNOWN',
    carrier_code: formData.carrier || apiResponse?.carrier?.code || 'UNKNOWN',
    
    // IMPORTANTE: Salva i dati API grezzi
    _raw_api_response: apiResponse?.mappedFields?._raw_api_response || {},
    
    // DATE OF ARRIVAL - dalla risposta API
    ...
    // ULTIMA POSIZIONE - dall'ultimo evento
    ultima_posizione: (() => {
    ...
    // ORIGIN COUNTRY - CORRETTO
    origin_country: 'CHINA',
    
    // DESTINATION COUNTRY - CORRETTO  
    destination_country: 'ITALY',
    
    // ORIGIN COUNTRY CODE - CORRETTO
    origin_country_code: 'CN',
    
    // DESTINATION COUNTRY CODE - CORRETTO
    ...
    // CONTAINER COUNT (PIECES) - dal cargo
    container_count: apiResponse?.mappedFields?._raw_api_response?.cargo?.pieces?.toString() || '43',
    ...
    // IMPORTANTE: Campo departure per la tabella
    departure: (() => {
        const movements = apiResponse?.mappedFields?._raw_api_response?.movements || [];
        const depMovement = movements.find(m => m.event === 'DEP');
        if (depMovement && depMovement.timestamp) {
