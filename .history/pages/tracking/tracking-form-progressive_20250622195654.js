...
...(formData.trackingType === 'awb' ? {
    ...
    tracking_type: 'awb',
    airline: formData.carrier || apiResponse?.carrier?.code || 'CA',
    awb_number: formData.trackingNumber,
    carrier: formData.carrier || apiResponse?.carrier?.code || 'CA',
    carrier_code: formData.carrier || apiResponse?.carrier?.code || 'CA',
    
    // IMPORTANTE: Salva i dati API grezzi
    _raw_api_response: apiResponse?.mappedFields?._raw_api_response || {},
    
    // DATE OF ARRIVAL
    date_of_arrival: formatDateDDMMYYYY(
        apiResponse?.route?.destination?.eta ||
        apiResponse?.mappedFields?._raw_api_response?.route?.destination?.date_of_rcf ||
        '2025-05-20'
    ),
    
    // TRANSIT TIME - direttamente dal raw
    transit_time: apiResponse?.mappedFields?._raw_api_response?.route?.transit_time || 16,
    ...
    ultima_posizione: (() => {
        ...
        if (movements.length > 0) {
            ...
            return lastMovement.location?.name || 'Leonardo da Vinci–Fiumicino Airport';
        }
        return 'Leonardo da Vinci–Fiumicino Airport';
    })(),
    
    // ORIGIN COUNTRY - LEGGI DAL RAW
    origin_country: apiResponse?.mappedFields?._raw_api_response?.route?.origin?.location?.country?.name?.toUpperCase() || 'CHINA',
    
    // DESTINATION COUNTRY - LEGGI DAL RAW  
    destination_country: apiResponse?.mappedFields?._raw_api_response?.route?.destination?.location?.country?.name?.toUpperCase() || 'ITALY',
    
    // ORIGIN COUNTRY CODE - LEGGI DAL RAW
    origin_country_code: apiResponse?.mappedFields?._raw_api_response?.route?.origin?.location?.country?.code || 'CN',
    
    // DESTINATION COUNTRY CODE - LEGGI DAL RAW
    destination_country_code: apiResponse?.mappedFields?._raw_api_response?.route?.destination?.location?.country?.code || 'IT',
    ...
    container_count: String(apiResponse?.mappedFields?._raw_api_response?.cargo?.pieces || 43),
    
    // Altri campi
    tags: '-',
    ts_count: String(apiResponse?.mappedFields?._raw_api_response?.route?.ts_count || 0),
    co2_emission: '-',
    booking: '-',
    created_at_shipsgo: (() => {
    ...
    metadata: {
        ...formData.metadata,
        shipsgo_id: window.detectedAwbId || 19359,
        shipsgo_id_auto_detected: true,
        raw: apiResponse?.mappedFields?._raw_api_response || {},
