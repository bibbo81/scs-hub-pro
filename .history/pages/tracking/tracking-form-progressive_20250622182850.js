...
// Fix 3: Aggiungi un fallback per detectTrackingType
function detectTrackingType(trackingNumber) {
    ...
    // Pattern per AWB (3 numeri + trattino + 8 numeri)
...
// Fix 2: Correggi la funzione di submit per gestire meglio gli errori
// Sostituisci la funzione handleEnhancedSubmit con questa versione più robusta:
...
async function handleEnhancedSubmit(e) {
    ...
    try {
        // Collect form data
        const formData = {
            trackingNumber: document.getElementById('enh_trackingNumber').value.trim().toUpperCase(),
            trackingType: document.getElementById('enh_trackingType').value,
            carrier: document.getElementById('enh_carrier').value,
            origin: document.getElementById('enh_origin').value.toUpperCase(),
            destination: document.getElementById('enh_destination').value.toUpperCase(),
        ...
        if (formData.trackingType === 'awb' && !formData.carrier) {
            const detectedAirline = detectAirlineFromAWB(formData.trackingNumber);
            if (detectedAirline) {
        ...
        try {
            ...
            if (result.success) {
                ...
                setTimeout(() => {
                    ...
                    setTimeout(() => {
                        ...
                        if (!table || table.children.length === 0) {
                            console.log('⚠️ Tabella vuota, ricarico la pagina...');
                            location.reload();
                        }
                    }, 1000);
                }, 2000);
                ...
                showWorkflowResult(false, result.message || 'Errore durante il salvataggio');
            }
        } catch (processError) {
            console.error('Process error:', processError);
            updateWorkflowStep(2, 'error', 'Errore');
            showWorkflowResult(false, processError.message || 'Errore imprevisto');
        }
    } catch (error) {
        ...
        if (window.TrackingErrorHandler) {
            window.TrackingErrorHandler.show({
                title: 'Errore',
...
// Fix 1: Aggiungi una funzione di utilità per formattare data e ora
// Aggiungi questa funzione DOPO formatDateDDMMYYYY (circa riga 1070)
...
function extractCountryCode(portName) {
   ...
   async function processEnhancedTracking(formData) {
    ...
    // 1. Analizza TUTTA la struttura
    console.log('1. STRUTTURA COMPLETA:');
    console.log(JSON.stringify(apiResponse, null, 2));
    
    // 2. Verifica dove sono i dati dei paesi
    console.log('\n2. RICERCA COUNTRY DATA:');
    
    // Funzione helper per cercare ricorsivamente
    function findKeysWithValue(obj, searchKey, path = '') {
        const results = [];
        ...
        for (const key in obj) {
            if (obj.hasOwnProperty(key)) {
                ...
                if (typeof obj[key] === 'object' && obj[key] !== null) {
                    findKeysWithValue(obj[key], searchKey, currentPath);
                }
            }
        }
    }
    
    findKeysWithValue(apiResponse);
    
    // 3. Cerca pieces/cargo
    console.log('\n3. RICERCA PIECES/CARGO:');
    function findPieces(obj, path = '') {
        for (const key in obj) {
            if (obj.hasOwnProperty(key)) {
                const currentPath = path ? `${path}.${key}` : key;
                
                if (key.toLowerCase().includes('piece') || 
                ...
                if (typeof obj[key] === 'object' && obj[key] !== null && !Array.isArray(obj[key])) {
                    findPieces(obj[key], currentPath);
                }
            }
        }
    }
    
    findPieces(apiResponse);
    ...
    function findDates(obj, path = '') {
        for (const key in obj) {
            if (obj.hasOwnProperty(key)) {
                ...
                if (typeof obj[key] === 'object' && obj[key] !== null && !Array.isArray(obj[key])) {
                    findDates(obj[key], currentPath);
                }
            }
        }
    }
    
    findDates(apiResponse);
    
    // 5. Verifica events array
    console.log('\n5. EVENTS ARRAY:');
    if (apiResponse.events && Array.isArray(apiResponse.events)) {
        console.log(`   Totale eventi: ${apiResponse.events.length}`);
        apiResponse.events.forEach((event, index) => {
...
if (apiResponse.events && Array.isArray(apiResponse.events)) {
   ...
   if (departureEvent) {
       ...
       const finalData = {
           ...
           // SOSTITUISCI il blocco AWB in finalData (tracking-form-progressive.js) con questo:
...
...(formData.trackingType === 'awb' ? {
    ...
    // Salva PRIMA i dati raw per usarli dopo
    _raw_api_response: apiResponse?.metadata?.raw?.shipment || apiResponse?.mappedFields?._raw_api_response || {},
    
    // DATE OF ARRIVAL - OK
    date_of_arrival: (() => {
        const arrivalDate = apiResponse?.route?.destination?.eta ||
                            apiResponse?.metadata?.date_of_arrival ||
                            apiResponse?.metadata?.raw?.shipment?.route?.destination?.date_of_rcf ||
                            formData.eta || '-';
    ...
    // TRANSIT TIME - OK
    transit_time: (() => {
        const rawTransit = apiResponse?.metadata?.raw?.shipment?.route?.transit_time;
        if (rawTransit) return rawTransit;
    ...
    ultima_posizione: (() => {
        if (apiResponse?.events && apiResponse.events.length > 0) {
            const lastEvent = apiResponse.events[0];
        ...
        const movements = apiResponse?.metadata?.raw?.shipment?.movements || [];
    ...
    // ORIGIN COUNTRY - OK
    origin_country: (() => {
        if (apiResponse?.metadata?.raw?.shipment?.route?.origin?.location?.country?.name) {
            return apiResponse.metadata.raw.shipment.route.origin.location.country.name.toUpperCase();
        }
    ...
    // DESTINATION COUNTRY - USA mappedFields._raw_api_response
    destination_country: (() => {
        // Prima prova in mappedFields (dove lo vediamo nel debug)
        if (apiResponse?.mappedFields?._raw_api_response?.route?.destination?.location?.country?.name) {
            return apiResponse.mappedFields._raw_api_response.route.destination.location.country.name.toUpperCase();
        }
        
        // Poi nei metadata raw
        if (apiResponse?.metadata?.raw?.shipment?.route?.destination?.location?.country?.name) {
            return apiResponse.metadata.raw.shipment.route.destination.location.country.name.toUpperCase();
        }
    ...
    // ORIGIN COUNTRY CODE - OK
    origin_country_code: (() => {
        if (apiResponse?.metadata?.raw?.shipment?.route?.origin?.location?.country?.code) {
            return apiResponse.metadata.raw.shipment.route.origin.location.country.code.toUpperCase();
        }
        
        const originPort = formData.origin || apiResponse?.route?.origin?.port || '-';
        const countryInfo = getCountryFromAirport(originPort);
        if (countryInfo.code !== '-') return countryInfo.code;
        
        return extractCountryCode(originPort);
    })(),
    
    // DESTINATION COUNTRY CODE - USA mappedFields._raw_api_response
    destination_country_code: (() => {
        // Prima prova in mappedFields (dove lo vediamo nel debug)
        if (apiResponse?.mappedFields?._raw_api_response?.route?.destination?.location?.country?.code) {
            return apiResponse.mappedFields._raw_api_response.route.destination.location.country.code.toUpperCase();
        }
        
        // Poi nei metadata raw
        if (apiResponse?.metadata?.raw?.shipment?.route?.destination?.location?.country?.code) {
            return apiResponse.metadata.raw.shipment.route.destination.location.country.code.toUpperCase();
        }
    ...
    // DATE OF DEPARTURE - USA movements o mappedFields
    date_of_departure: (() => {
        // Prima cerca in mappedFields movements
        if (apiResponse?.mappedFields?._raw_api_response?.movements) {
            const depMovement = apiResponse.mappedFields._raw_api_response.movements.find(m => m.event === 'DEP');
            if (depMovement && depMovement.timestamp) {
                return formatDateDDMMYYYY(depMovement.timestamp);
            }
        }
        
        // Poi nei metadata raw movements
        if (apiResponse?.metadata?.raw?.shipment?.movements) {
            const depMovement = apiResponse.metadata.raw.shipment.movements.find(m => m.event === 'DEP');
            if (depMovement && depMovement.timestamp) {
                return formatDateDDMMYYYY(depMovement.timestamp);
            }
        }
        
        // Usa date_of_dep dal route
        const depDate = apiResponse?.mappedFields?._raw_api_response?.route?.origin?.date_of_dep ||
                        apiResponse?.metadata?.raw?.shipment?.route?.origin?.date_of_dep;
        if (depDate) {
    ...
    // CONTAINER COUNT (PIECES) - USA cargo.pieces da mappedFields
    container_count: (() => {
        // Prima cerca in mappedFields
        if (apiResponse?.mappedFields?._raw_api_response?.cargo?.pieces) {
            return apiResponse.mappedFields._raw_api_response.cargo.pieces.toString();
        }
        
        // Poi nei metadata raw
        if (apiResponse?.metadata?.raw?.shipment?.cargo?.pieces) {
            return apiResponse.metadata.raw.shipment.cargo.pieces.toString();
        }
    ...
    tags: formData.tags || '-',
    ts_count: (() => {
        if (apiResponse?.mappedFields?._raw_api_response?.route?.ts_count !== undefined) {
            return apiResponse.mappedFields._raw_api_response.route.ts_count.toString();
        }
        if (apiResponse?.metadata?.raw?.shipment?.route?.ts_count !== undefined) {
            return apiResponse.metadata.raw.shipment.route.ts_count.toString();
        }
        return '0';
    })(),
    co2_emission: '-',
    booking: '-',
    created_at_shipsgo: (() => {
        const createdAt = apiResponse?.mappedFields?._raw_api_response?.created_at ||
                         apiResponse?.metadata?.raw?.shipment?.created_at;
        if (createdAt) {
            return createdAt.replace(' ', ' ').split(' ')[0].split('-').reverse().join('/');
        }
        return formatDateDDMMYYYY(new Date().toISOString());
    })(),
    
    // Salva metadata con ShipsGo ID se rilevato
    ...(window.detectedAwbId ? {
        metadata: {
            ...formData.metadata,
            shipsgo_id: window.detectedAwbId,
            shipsgo_id_auto_detected: true,
            raw: apiResponse?.metadata?.raw || {}
        }
    } : {
        metadata: {
            ...formData.metadata,
   ...
   window.QuickContainerActions = {
       async checkAndShowActions(containerNumber) {
           ...
           if (existingContainer) {
               this.showQuickActthis.showQuickActionsModal(existingContainer);
              return true;
          }
          return false;
      },
