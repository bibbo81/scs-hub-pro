...
...(formData.trackingType === 'awb' ? {
    ...
    // DATE OF ARRIVAL - OK nel tuo fix
    date_of_arrival: (() => {
    ...
    // TRANSIT TIME - OK nel tuo fix
    transit_time: (() => {
        // Prima cerca nei dati raw
    ...
    // ULTIMA POSIZIONE - OK nel tuo fix
    ultima_posizione: (() => {
        ...
        // Cerca nell'ultimo movement raw
    ...
    // FIX ORIGIN COUNTRY - USA I DATI RAW CORRETTI
    origin_country: (() => {
        // Il percorso CORRETTO nei dati raw è metadata.raw.shipment (non apiResponse.metadata)
    ...
    // FIX ORIGIN COUNTRY CODE - USA I DATI RAW CORRETTI
    origin_country_code: (() => {
        // Il percorso CORRETTO nei dati raw è metadata.raw.shipment
    ...
    // FIX DESTINATION COUNTRY CODE - USA I DATI RAW CORRETTI
    ...
    // FIX DATE OF DEPARTURE - USA I MOVEMENTS
    date_of_departure: (() => {
        // 1. Cerca nell'evento DEP nei movements raw
        ...
        // 2. Usa date_of_dep dal route origin
        if (apiResponse?.metadata?.raw?.shipment?.route?.origin?.date_of_dep) {
            return formatDateDDMMYYYY(apiResponse.metadata.raw.shipment.route.origin.date_of_dep);
        }
        
        // 3. Cerca negli eventi normali
        if (apiResponse?.events && apiResponse.events.length > 0) {
    ...
    // FIX CONTAINER COUNT (PIECES) - USA CARGO.PIECES
    container_count: (() => {
        // Il percorso CORRETTO nei dati raw è metadata.raw.shipment.cargo.pieces
    ...
    created_at_shipsgo: (() => {
        // Usa created_at dai dati raw
        const createdAt = apiResponse?.metadata?.raw?.shipment?.created_at;
        if (createdAt) {
            // Già nel formato corretto "2025-05-21 09:46:47"
            return createdAt.replace(' ', ' ').split(' ')[0].split('-').reverse().join('/');
        }
        return formatDateDDMMYYYY(new Date().toISOString());
    })(),
    
    // Salva TUTTI i dati raw per debug futuro
    _raw_api_response: apiResponse?.metadata?.raw?.shipment || {},
