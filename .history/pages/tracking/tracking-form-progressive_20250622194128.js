...
...(formData.trackingType === 'awb' ? {
    ...
    date_of_arrival: (() => {
        const arrivalDate = apiResponse?.route?.destination?.eta ||
                            ...
                            apiResponse?.mappedFields?._raw_api_response?.route?.destination?.date_of_rcf ||
                            formData.eta || '-';
    ...
    // TRANSIT TIME (ore)
    transit_time: (() => {
        const rawTransit = apiResponse?.mappedFields?._raw_api_response?.route?.transit_time ||
                          apiResponse?.metadata?.raw?.shipment?.route?.transit_time;
    ...
    ultima_posizione: (() => {
        ...
        // Poi nei movements
        const movements = apiResponse?.mappedFields?._raw_api_response?.movements || 
                         apiResponse?.metadata?.raw?.shipment?.movements || [];
    ...
    origin_country: (() => {
        ...
        const paths = [
            apiResponse?.mappedFields?._raw_api_response?.route?.origin?.location?.country?.name,
            apiResponse?.metadata?.raw?.shipment?.route?.origin?.location?.country?.name,
            apiResponse?.route?.origin?.country,
            apiResponse?.metadata?.origin_country
        ];
    ...
    destination_country: (() => {
        // Prova tutti i possibili percorsi
        const paths = [
            apiResponse?.mappedFields?._raw_api_response?.route?.destination?.location?.country?.name,
            apiResponse?.metadata?.raw?.shipment?.route?.destination?.location?.country?.name,
            apiResponse?.route?.destination?.country,
            apiResponse?.metadata?.destination_country
        ];
    ...
    origin_country_code: (() => {
        const paths = [
            apiResponse?.mappedFields?._raw_api_response?.route?.origin?.location?.country?.code,
            apiResponse?.metadata?.raw?.shipment?.route?.origin?.location?.country?.code,
            apiResponse?.route?.origin?.country_code,
            apiResponse?.metadata?.origin_country_code
        ];
    ...
    destination_country_code: (() => {
        const paths = [
            apiResponse?.mappedFields?._raw_api_response?.route?.destination?.location?.country?.code,
            apiResponse?.metadata?.raw?.shipment?.route?.destination?.location?.country?.code,
            apiResponse?.route?.destination?.country_code,
            apiResponse?.metadata?.destination_country_code
        ];
    ...
    date_of_departure: (() => {
        ...
        return formatDateDDMMYYYY(yesterday.toISOString());
    })(),
    
    // CONTAINER COUNT (PIECES per AWB)
    container_count: (() => {
        const paths = [
            apiResponse?.mappedFields?._raw_api_response?.cargo?.pieces,
            apiResponse?.metadata?.raw?.shipment?.cargo?.pieces,
            apiResponse?.package?.pieces,
            apiResponse?.metadata?.pieces
        ];
    ...
    tags: formData.tags || '-',
    ts_count: (() => {
        const tsCount = apiResponse?.mappedFields?._raw_api_response?.route?.ts_count ||
                       apiResponse?.metadata?.raw?.shipment?.route?.ts_count;
        return tsCount !== undefined ? tsCount.toString() : '0';
    })(),
    co2_emission: '-', // Non applicabile per AWB
    booking: '-',
    created_at_shipsgo: (() => {
        ...
        if (createdAt && createdAt.includes(' ')) {
            // Formato: "2025-05-21 11:46:47" -> "21/05/2025"
            return createdAt.split(' ')[0].split('-').reverse().join('/');
        }
        return formatDateDDMMYYYY(createdAt);
    })(),
    
    // Aggiungi alias per la tabella
    departure: (() => {
        // Stesso valore di date_of_departure
        const movements = apiResponse?.mappedFields?._raw_api_response?.movements ||
                         apiResponse?.metadata?.raw?.shipment?.movements || [];
    ...
    // Metadata con ShipsGo ID se rilevato
    metadata: {
        ...formData.metadata,
