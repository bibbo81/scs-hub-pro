...
if (apiResponse.events && Array.isArray(apiResponse.events)) {
   ...
   if (departureEvent) {
    ...
    origin_country: (() => {
        // Il percorso CORRETTO nei dati raw
        const rawCountry = apiResponse?.metadata?.raw?.shipment?.route?.origin?.location?.country?.name;
    ...
    destination_country: (() => {
        // Il percorso CORRETTO nei dati raw
        const rawCountry = apiResponse?.metadata?.raw?.shipment?.route?.destination?.location?.country?.name;
        if (rawCountry) return rawCountry.toUpperCase();
    ...
    origin_country_code: (() => {
        // Il percorso CORRETTO nei dati raw
        const rawCode = apiResponse?.metadata?.raw?.shipment?.route?.origin?.location?.country?.code;
    ...
    destination_country_code: (() => {
        // Il percorso CORRETTO nei dati raw
        const rawCode = apiResponse?.metadata?.raw?.shipment?.route?.destination?.location?.country?.code;
        if (rawCode) return rawCode.toUpperCase();
    ...
    date_of_departure: (() => {
        ...
        const depDate = apiResponse?.metadata?.raw?.shipment?.route?.origin?.date_of_dep;
        if (depDate) {
    ...
    container_count: (() => {
        // Il percorso CORRETTO nei dati raw
        const rawPieces = apiResponse?.metadata?.raw?.shipment?.cargo?.pieces;
        if (rawPieces) return rawPieces.toString();
    ...
    ts_count: (() => {
        ...
        const tsCount = apiResponse?.metadata?.raw?.shipment?.route?.ts_count;
