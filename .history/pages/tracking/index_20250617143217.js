...
function getColumnFormatters() {
    return {
        // ===== FUNZIONE HELPER PER TROVARE VALORI =====
        ...
        tracking_number: function(value, row) {
            ...
            // Determina il tipo automaticamente
            let type = helpers.getValue(row, ['tracking_type', 'TIPO', 'Type']);
            if (!type) {
                if (number.includes('-') || helpers.getValue(row, ['AIRLINE', 'Airline'])) {
                    type = 'awb';
                } else if (helpers.getValue(row, ['CONTAINER COUNT', 'Container Count', 'PORT OF LOADING'])) {
                    type = 'container';
                } else {
        ...
        tracking_type: function(value, row) {
            ...
            const badgeClass = (type === 'awb' || type === 'air') ? 'badge-warning' : 'badge-info';
        ...
        status: function(value, row) {
            ...
            // ===== STATUS MAPPING COMPLETO DA SHIPSGO =====
            const statusMapping = {
                // Maritime
                'Sailing': 'In transito',
                'Arrived': 'Arrivata',
            ...
            if (mappedStatus.toLowerCase().includes('consegnato')) badgeClass = 'badge-success';
            else if (mappedStatus.toLowerCase().includes('transito') || mappedStatus.toLowerCase().includes('arrivata')) badgeClass = 'badge-info';
            else if (mappedStatus.toLowerCase().includes('consegna') || mappedStatus.toLowerCase().includes('sdoganata')) badgeClass = 'badge-warning';
            else if (mappedStatus.toLowerCase().includes('ritardo') || mappedStatus.toLowerCase().includes('eccezione')) badgeClass = 'badge-danger';
            else if (mappedStatus.toLowerCase().includes('creata') || mappedStatus.toLowerCase().includes('ricevuto')) badgeClass = 'badge-secondary';
...
// ===== HELPER FUNCTION PER DATE FORMATTING =====
