...
// ===== FIX FORMATTER COLONNE - TRACKING TABLE =====
function getColumnFormatters() {
    return {
        // ===== BASIC FIELDS =====
        tracking_number: (value, row) => {
            const number = value || row.tracking_number || row.metadata?.tracking_number || '-';
            const type = row.tracking_type || row.metadata?.tracking_type || 'container';
            const typeIcon = type === 'awb' ? '✈️' : (type === 'container' ? '📦' : '🚛');
            return `<span class="tracking-number">${typeIcon} ${number}</span>`;
        },

        tracking_type: (value, row) => {
            const type = value || row.tracking_type || row.metadata?.tracking_type || row.metadata?.import_type || 'container';
            const typeMap = {
                'awb': 'AEREO',
                'container': 'MARE', 
                'bl': 'MARE',
            ...
            return `<span class="sol-badge ${badgeClass}">${label}</span>`;
        },

        carrier_code: (value, row) => {
            return value || row.carrier_code || 
                   row.metadata?.carrier_code || row.metadata?.Carrier || 
                   row.metadata?.Airline || row.metadata?.airline || '-';
        },

        status: (value, row) => {
            const status = value || row.status || 'registered';
            const statusMap = {
                'registered': { label: 'Registrato', class: 'badge-secondary' },
                'in_transit': { label: 'In Transito', class: 'badge-info' },
                'arrived': { label: 'Arrivato', class: 'badge-primary' },
                'customs_cleared': { label: 'Sdoganato', class: 'badge-warning' },
                'out_for_delivery': { label: 'In Consegna', class: 'badge-warning' },
                'delivered': { label: 'Consegnato', class: 'badge-success' },
                'delayed': { label: 'In Ritardo', class: 'badge-danger' },
                'exception': { label: 'Eccezione', class: 'badge-danger' }
            };
            const statusInfo = statusMap[status] || { label: status, class: 'badge-secondary' };
            return `<span class="sol-badge ${statusInfo.class}">${statusInfo.label}</span>`;
        },

        // ===== ORIGIN/DESTINATION =====
        origin_port: (value, row) => {
            return value || row.origin_port || 
                   row.metadata?.origin_port || row.metadata?.Origin || 
                   row.metadata?.origin || row.metadata?.['Port Of Loading'] || 
                   row.metadata?.port_of_loading || '-';
        },

        destination_port: (value, row) => {
            return value || row.destination_port || 
                   row.metadata?.destination_port || row.metadata?.Destination || 
                   row.metadata?.destination || row.metadata?.['Port Of Discharge'] || 
                   row.metadata?.port_of_discharge || '-';
        },

        // ===== AIR SPECIFIC FORMATTERS =====
        awb_number: (value, row) => {
            return value || row.metadata?.['AWB Number'] || 
                   row.metadata?.awb_number || row.tracking_number || '-';
        },

        airline: (value, row) => {
            return value || row.metadata?.Airline || 
                   row.metadata?.airline || row.carrier_code || '-';
        },

        origin_country: (value, row) => {
            return value || row.metadata?.['Origin Country'] || 
                   row.metadata?.origin_country || row.metadata?.['POL Country'] || 
                   row.metadata?.pol_country || '-';
        },

        origin_country_code: (value, row) => {
            const code = value || row.metadata?.['Origin Country Code'] || 
                        row.metadata?.origin_country_code || row.metadata?.['POL Country Code'] || 
                        row.metadata?.pol_country_code || '';
            return code ? `<span class="country-code">${code}</span>` : '-';
        },

        destination_country: (value, row) => {
            return value || row.metadata?.['Destination Country'] || 
                   row.metadata?.destination_country || row.metadata?.['POD Country'] || 
                   row.metadata?.pod_country || '-';
        },

        destination_country_code: (value, row) => {
            const code = value || row.metadata?.['Destination Country Code'] || 
                        row.metadata?.destination_country_code || row.metadata?.['POD Country Code'] || 
                        row.metadata?.pod_country_code || '';
            return code ? `<span class="country-code">${code}</span>` : '-';
        },

        date_of_departure: (value, row) => {
            const date = value || row.metadata?.['Date Of Departure'] || 
                        row.metadata?.date_of_departure || row.departure_date || '';
            return date ? formatDate(date) : '-';
        },

        date_of_arrival: (value, row) => {
            const date = value || row.metadata?.['Date Of Arrival'] || 
                        row.metadata?.date_of_arrival || row.eta || '';
            return date ? formatDate(date) : '-';
        },

        transit_time: (value, row) => {
            const time = value || row.metadata?.['Transit Time'] || 
                        row.metadata?.transit_time || row.metadata?.transit_days || '';
            return time ? `${time} giorni` : '-';
        },

        t5_count: (value, row) => {
            const count = value || row.metadata?.['T5 Count'] || 
        ...
        reference: (value, row) => {
            return value || row.metadata?.Reference || 
                   row.metadata?.reference || row.tracking_number || '-';
        },

        booking: (value, row) => {
            return value || row.metadata?.Booking || 
                   row.metadata?.booking || row.metadata?.reference || '-';
        },

        container_count: (value, row) => {
            const count = value || row.metadata?.['Container Count'] || 
                         row.metadata?.container_count || '';
            return count ? `${count} container${count > 1 ? 's' : ''}` : '-';
        },

        port_of_loading: (value, row) => {
            return value || row.metadata?.['Port Of Loading'] || 
                   row.metadata?.port_of_loading || row.origin_port || '-';
        },

        date_of_loading: (value, row) => {
            const date = value || row.metadata?.['Date Of Loading'] || 
                        row.metadata?.date_of_loading || row.departure_date || '';
            return date ? formatDate(date) : '-';
        },

        port_of_discharge: (value, row) => {
            return value || row.metadata?.['Port Of Discharge'] || 
                   row.metadata?.port_of_discharge || row.destination_port || '-';
        },

        date_of_discharge: (value, row) => {
            const date = value || row.metadata?.['Date Of Discharge'] || 
                        row.metadata?.date_of_discharge || row.eta || '';
            return date ? formatDate(date) : '-';
        },

        pol_country: (value, row) => {
            return value || row.metadata?.['POL Country'] || 
                   row.metadata?.pol_country || row.metadata?.['Origin Country'] || 
                   row.metadata?.origin_country || '-';
        },

        pol_country_code: (value, row) => {
            const code = value || row.metadata?.['POL Country Code'] || 
                        row.metadata?.pol_country_code || row.metadata?.['Origin Country Code'] || 
                        row.metadata?.origin_country_code || '';
            return code ? `<span class="country-code">${code}</span>` : '-';
        },

        pod_country: (value, row) => {
            return value || row.metadata?.['POD Country'] || 
                   row.metadata?.pod_country || row.metadata?.['Destination Country'] || 
                   row.metadata?.destination_country || '-';
        },

        pod_country_code: (value, row) => {
            const code = value || row.metadata?.['POD Country Code'] || 
                        row.metadata?.pod_country_code || row.metadata?.['Destination Country Code'] || 
                        row.metadata?.destination_country_code || '';
            return code ? `<span class="country-code">${code}</span>` : '-';
        },

        co2_emission: (value, row) => {
            const emission = value || row.metadata?.['CO₂ Emission (Tons)'] || 
                           row.metadata?.co2_emission || '';
            return emission ? `${emission} ton CO₂` : '-';
        },

        tags: (value, row) => {
            const tags = value || row.metadata?.Tags || row.metadata?.tags || '';
            if (tags && typeof tags === 'string') {
                return tags.split(',').map(tag => 
                    `<span class="sol-badge badge-light">${tag.trim()}</span>`
                ).join(' ');
            }
            return tags || '-';
        },
        ...
        // ===== COMMON FIELDS =====
        eta: (value, row) => {
            const eta = value || row.eta || 
                       row.metadata?.['Date Of Arrival'] || row.metadata?.date_of_arrival ||
                       row.metadata?.['Date Of Discharge'] || row.metadata?.date_of_discharge || '';
            return eta ? formatDate(eta) : '-';
        },

        ultima_posizione: (value, row) => {
            const position = value || row.ultima_posizione || 
                           row.last_known_location || row.last_event_location ||
                           row.destination_port || row.metadata?.destination || 
