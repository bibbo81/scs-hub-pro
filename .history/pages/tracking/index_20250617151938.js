...
// ===== FORMATTERS FIX - GESTIONE METADATA E DATI DIRETTI =====
// Sostituisci SOLO la funzione getColumnFormatters() nel tuo index.js
...
function getColumnFormatters() {
    return {
        ...
        tracking_number: (value, row) => {
            const number = value || row.tracking_number || 
                          row.metadata?.['AWB Number'] || row['AWB Number'] || 
                          row.metadata?.['Reference'] || row['Reference'] || 
                          row.metadata?.tracking_number || '-';
            
            const type = row.tracking_type || row['tracking_type'] || 
                        (row.metadata?.['AWB Number'] || row['AWB Number'] ? 'awb' : 'container');
        ...
        tracking_type: (value, row) => {
            ...
            let type = value || row.tracking_type || row['tracking_type'] || row.metadata?.tracking_type;
            
            // Auto-detection se non specificato
            if (!type) {
                if (row.metadata?.['AWB Number'] || row['AWB Number'] || 
                    row.metadata?.['Airline'] || row['Airline']) {
                    type = 'awb';
                } else if (row.metadata?.['Container Count'] || row['Container Count'] || 
                          row.metadata?.['Port Of Loading'] || row['Port Of Loading']) {
                    type = 'container';
                } else {
        ...
        carrier_code: (value, row) => {
            return value || row.carrier_code || 
                   row.metadata?.['Carrier'] || row['Carrier'] || 
                   row.metadata?.['Airline'] || row['Airline'] || '-';
        },

        status: (value, row) => {
            const originalStatus = value || row.status || 
                                 row.metadata?.['Status'] || row['Status'] || 'registered';
        ...
        origin_port: (value, row) => {
            return value || row.origin_port || 
                   row.metadata?.['Origin'] || row['Origin'] || 
                   row.metadata?.['Port Of Loading'] || row['Port Of Loading'] || '-';
        },

        destination_port: (value, row) => {
            return value || row.destination_port || 
                   row.metadata?.['Destination'] || row['Destination'] || 
                   row.metadata?.['Port Of Discharge'] || row['Port Of Discharge'] || '-';
        },
        ...
        origin_country: (value, row) => {
            return value || 
                   row.metadata?.['Origin Country'] || row['Origin Country'] || 
                   row.metadata?.['POL Country'] || row['POL Country'] || '-';
        },

        origin_country_code: (value, row) => {
            const code = value || 
                        row.metadata?.['Origin Country Code'] || row['Origin Country Code'] || 
                        row.metadata?.['POL Country Code'] || row['POL Country Code'] || '';
        ...
        destination_country: (value, row) => {
            return value || 
                   row.metadata?.['Destination Country'] || row['Destination Country'] || 
                   row.metadata?.['POD Country'] || row['POD Country'] || '-';
        },

        destination_country_code: (value, row) => {
            const code = value || 
                        row.metadata?.['Destination Country Code'] || row['Destination Country Code'] || 
                        row.metadata?.['POD Country Code'] || row['POD Country Code'] || '';
            return code ? `<span class="country-code" style="background: #f0f0f0; padding: 2px 6px; border-radius: 4px; font-size: 10px; font-weight: bold;">${code}</span>` : '-';
        },

        date_of_departure: (value, row) => {
            const date = value || 
                        row.metadata?.['Date Of Departure'] || row['Date Of Departure'] || 
                        row.departure_date || '';
        ...
        date_of_arrival: (value, row) => {
            const date = value || 
                        row.metadata?.['Date Of Arrival'] || row['Date Of Arrival'] || 
                        row.eta || '';
            return date ? formatDate(date) : '-';
        },
        ...
        transit_time: (value, row) => {
            ...
            let time = value || 
                      row.metadata?.['Transit Time'] || row['Transit Time'] || 
                      row.metadata?.transit_time || row.transit_time || '';
            
            // Calcola transit time se abbiamo le date
            if (!time) {
                const depDate = row.metadata?.['Date Of Departure'] || row['Date Of Departure'];
                const arrDate = row.metadata?.['Date Of Arrival'] || row['Date Of Arrival'];
        ...
        t5_count: (value, row) => {
            return value || 
                   row.metadata?.['T5 Count'] || row['T5 Count'] || '-';
        },
        ...
        booking: (value, row) => {
            return value || 
                   row.metadata?.['Booking'] || row['Booking'] || '-';
        },

        container_count: (value, row) => {
            const count = value || 
                         row.metadata?.['Container Count'] || row['Container Count'] || '';
        ...
        date_of_loading: (value, row) => {
            const date = value || 
                        row.metadata?.['Date Of Loading'] || row['Date Of Loading'] || 
                        row.departure_date || '';
            return date ? formatDate(date) : '-';
        },
        ...
        date_of_discharge: (value, row) => {
            const date = value || 
                        row.metadata?.['Date Of Discharge'] || row['Date Of Discharge'] || 
                        row.eta || '';
            return date ? formatDate(date) : '-';
        },
        ...
        co2_emission: (value, row) => {
            const emission = value || 
                           row.metadata?.['CO₂ Emission (Tons)'] || row['CO₂ Emission (Tons)'] || '';
            return emission ? `${emission} ton CO₂` : '-';
        },

        tags: (value, row) => {
            const tags = value || 
                        row.metadata?.['Tags'] || row['Tags'] || '';
