...
function getColumnFormatter(key) {
    const formatters = {
        ...
        eta: (value, row) => {
            // 1. Prima prova il valore ETA salvato
            let etaValue = value || row.eta;
            
            // 2. Per container DISCHARGED, usa Date Of Discharge (già successo)
            ...
            // 3. Per container in transito, usa Date Of Discharge come ETA futuro
            ...
            // 4. Per spedizioni aeree, usa Date Of Arrival come ETA
            else if (row.tracking_type === 'awb' && (!etaValue || etaValue === '-')) {
            ...
            if (/^\d{1,2}\/\d{1,2}\/\d{4}$/.test(etaValue)) {
                ...
                // Per container discharged, non mostrare icona futuro
        ...
        last_event_location: (value, row) => {
            // Se c'è un valore diretto, usalo
            ...
            // Per spedizioni aeree, cerca origin/destination names
            if (row.tracking_type === 'awb') {
                const origin = row.metadata?.origin_name || 
                              row.metadata?.['Origin Name'] ||
                              row.metadata?.['Destination Name'] || // Se in transito verso destinazione
                              row.metadata?.destination_name;
                if (origin) return origin;
            }
            
            // Fallback per tutte le spedizioni
