...
// ===== FORMATTER DELLE COLONNE - COMPLETI E PERFETTI CON SOLARIUM DESIGN =====
function getColumnFormatter(key) {
    const formatters = {
        tracking_type: (value) => {
            const types = {
                'container': { icon: 'fa-cube', text: 'MARE', class: 'sol-badge-info' },
                'bl': { icon: 'fa-file-alt', text: 'B/L', class: 'sol-badge-info' },
                'awb': { icon: 'fa-plane', text: 'AEREO', class: 'sol-badge-warning' },
                'air': { icon: 'fa-plane', text: 'AEREO', class: 'sol-badge-warning' },
                'parcel': { icon: 'fa-box', text: 'PACCO', class: 'sol-badge-success' }
            };
            const config = types[value] || { icon: 'fa-question', text: value?.toUpperCase() || 'N/A', class: 'sol-badge-secondary' };
            return `<span class="sol-badge ${config.class}"><i class="fas ${config.icon}"></i> ${config.text}</span>`;
        },
        
        status: (value) => {
            const statuses = {
                'registered': { class: 'sol-badge-secondary', text: 'Registrato', icon: 'fa-clock' },
                'in_transit': { class: 'sol-badge-info', text: 'In Transito', icon: 'fa-ship' },
                'arrived': { class: 'sol-badge-primary', text: 'Arrivato', icon: 'fa-anchor' },
                'customs_cleared': { class: 'sol-badge-success', text: 'Sdoganato', icon: 'fa-check' },
                'out_for_delivery': { class: 'sol-badge-warning', text: 'In Consegna', icon: 'fa-truck' },
                'delivered': { class: 'sol-badge-success', text: 'Consegnato', icon: 'fa-check-circle' },
                'delayed': { class: 'sol-badge-danger', text: 'In Ritardo', icon: 'fa-exclamation-triangle' },
                'exception': { class: 'sol-badge-danger', text: 'Eccezione', icon: 'fa-times-circle' }
            };
            const config = statuses[value] || { class: 'sol-badge-secondary', text: value || 'Sconosciuto', icon: 'fa-question' };
            return `<span class="sol-badge ${config.class}"><i class="fas ${config.icon}"></i> ${config.text}</span>`;
        },
        
        carrier_code: (value) => value || '-',
        
        // ===== FORMATTER INTELLIGENTI PER SHIPSGO - COMPLETI =====
        ...
        // ===== FORMATTER SPECIFICI SHIPSGO AIR =====
            ...
            if (depDate && arrDate) {
                try {
                    ...
                    if (!isNaN(dep.getTime()) && !isNaN(arr.getTime())) {
                        ...
                        return `${finalDays}`; // Solo il numero
                    }
                } catch (e) {
        ...
        // ===== FORMATTER SPECIFICI SHIPSGO SEA =====
        booking: (value, row) => {
            return value || 
                   row.metadata?.booking || 
                   row.metadata?.['Booking'] || 
        ...
        eta: (value, row) => {
            ...
            // Per container DISCHARGED, usa Date Of Discharge
            if ((row.tracking_type === 'container' || row.tracking_type === 'bl') && 
                row.metadata?.Status === 'Discharged') {
                etaValue = row.metadata?.['Date Of Discharge'] || row.metadata?.date_of_discharge;
            }
            // Per container in transito, usa Date Of Discharge come ETA futuro
            else if ((row.tracking_type === 'container' || row.tracking_type === 'bl') && 
                     (!etaValue || etaValue === '-')) {
                etaValue = row.metadata?.['Date Of Discharge'] || 
                          row.metadata?.date_of_discharge ||
                          row.metadata?.['Date Of Loading'] ||
                          row.metadata?.date_of_loading;
            }
            // Per spedizioni aeree, usa Date Of Arrival come ETA
            else if ((row.tracking_type === 'awb' || row.tracking_type === 'air') && (!etaValue || etaValue === '-')) {
                etaValue = row.metadata?.['Date Of Arrival'] || 
                          row.metadata?.date_of_arrival;
            }
            
            if (!etaValue || etaValue === '-') return '-';
            
            // Pulisci il formato se contiene orario
            if (typeof etaValue === 'string' && etaValue.includes(' ')) {
                etaValue = etaValue.split(' ')[0];
            }
            
            // Se già in formato DD/MM/YYYY, verifica se è futuro
            if (/^\d{1,2}\/\d{1,2}\/\d{4}$/.test(etaValue)) {
                const [day, month, year] = etaValue.split('/');
                const date = new Date(year, month - 1, day);
                const now = new Date();
                const isInFuture = date > now;
                
                const showClock = isInFuture && !(row.metadata?.Status === 'Discharged');
                
                return `<span class="${isInFuture ? 'text-primary' : 'text-muted'}">
                    ${etaValue}
                    ${showClock ? ' <i class="fas fa-clock"></i>' : ''}
                </span>`;
            }
            
            // Converti da ISO o altro formato
            try {
        ...
        // ===== ULTIMA POSIZIONE INTELLIGENTE =====
        last_event_location: (value, row) => {
            if (value && value !== '-') return value;
            
            // Per container con status "Discharged", usa Port Of Discharge
            if ((row.tracking_type === 'container' || row.tracking_type === 'bl') && 
                row.metadata?.Status === 'Discharged') {
                const port = row.metadata?.['Port Of Discharge'] || row.metadata?.port_of_discharge;
                if (port) return port;
            }
            
            // Per spedizioni aeree
            if (row.tracking_type === 'awb' || row.tracking_type === 'air') {
                const origin = row.metadata?.origin_name || 
            ...
            // Fallback
            const location = row.last_event_location || 
    ...
    // Formatter specifico o universale di fallback
...
// Helper per formattare le date - PERFETTO
