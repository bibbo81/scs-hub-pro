...
{ key: 'port_of_loading', label: 'Port Of Loading', visible: false, order: 10 },{ key: 'pol_country_code', label: 'POL Country Code', visible: false, order: 13 },  // ← NASCOSTA
...
{ key: 'origin_name', label: 'Origin Name', visible: true, order: 24 },
...
{ key: 'destination', label: 'Destination', visible: true, order: 28 },
...
function showColumnManager() {
    ...
    const columnsToHide = ['pol_country', 'pol_country_code', 'pod_country', 'pod_country_code', 'port_of_loading', 'port_of_discharge'];
...
function getColumnFormatter(key) {
    const formatters = {
        ...
        origin_port: (value, row) => {
            // Per AIR: usa Origin dal metadata
        ...
        destination_port: (value, row) => {
            // Per AIR: usa Destination dal metadata
            if (row.tracking_type === 'awb') {
                return row.metadata?.['Destination'] || 
        ...
        // ORIGIN NAME - UNIFICATO PER AIR E SEA
...
// DESTINATION NAME - UNIFICATO PER AIR E SEA
