...
(function() {
    ...
    function getCountryFromAirport(airportCode) {
        ...
        return AIRPORT_TO_COUNTRY[airportCode.toUpperCase()] || {
            name: '-',
            code: '-'
        };
    }
    ...
    async function processEnhancedTracking(formData) {
            ...
            try {
                if (formData.apiOperation === 'get' || formData.apiOperation === 'auto') {
                    ...
                    if (apiResponse && apiResponse.success && formData.trackingType === 'awb') {
                        console.log('🔍 DEBUG AWB API Response:');
                        console.log('=====================================');
                        // 1. Struttura completa
                        console.log('1. API Response completa:', apiResponse);
                        // 2. Metadata raw
                        console.log('2. Metadata RAW:', apiResponse.metadata?.raw);
                        // 3. Route info
                        console.log('3. Route info:', {
                            origin: apiResponse.route?.origin,
                            destination: apiResponse.route?.destination,
                            raw_route: apiResponse.metadata?.raw?.route
                        });
                        // 4. Cargo/Package info
                        console.log('4. Cargo/Package:', {
                            package: apiResponse.package,
                            cargo: apiResponse.metadata?.raw?.cargo,
                            pieces_locations: {
                                'package.pieces': apiResponse.package?.pieces,
                                'metadata.pieces': apiResponse.metadata?.pieces,
                                'raw.cargo.pieces': apiResponse.metadata?.raw?.cargo?.pieces
                            }
                        });
                        // 5. Movements per departure date
                        console.log('5. Movements:', apiResponse.metadata?.raw?.movements);
                        // 6. Country info
                        console.log('6. Country info:', {
                            origin_country_raw: apiResponse.metadata?.raw?.route?.origin?.location?.country,
                            destination_country_raw: apiResponse.metadata?.raw?.route?.destination?.location?.country,
                            origin_code_raw: apiResponse.metadata?.raw?.route?.origin?.location?.country_code,
                            destination_code_raw: apiResponse.metadata?.raw?.route?.destination?.location?.country_code
                        });
                        console.log('=====================================');
                    }
        ...
        const finalData = {
            ...
            ...(formData.trackingType === 'awb' ? {
                ...
                // DATE OF ARRIVAL - Corretto dal tuo fix
                date_of_arrival: (() => {
                ...
                // ULTIMA POSIZIONE - Corretto dal tuo fix
                ultima_posizione: (() => {
                ...
                // FIX 1: ORIGIN E DESTINATION COUNTRY DIRETTAMENTE DAI DATI API
                origin_country: (() => {
                    // 1. Prova dai dati raw dell'API
                    if (apiResponse?.metadata?.raw?.route?.origin?.location?.country) {
                        return apiResponse.metadata.raw.route.origin.location.country.toUpperCase();
                    }
                    // 2. Prova dal route normale
                    if (apiResponse?.route?.origin?.country) {
                        return apiResponse.route.origin.country.toUpperCase();
                    }
                    // 3. Deriva dall'aeroporto IATA
                ...
                destination_country: (() => {
                    // 1. Prova dai dati raw dell'API
                    if (apiResponse?.metadata?.raw?.route?.destination?.location?.country) {
                        return apiResponse.metadata.raw.route.destination.location.country.toUpperCase();
                    }
                    // 2. Prova dal route normale
                ...
                // FIX 2: COUNTRY CODES CORRETTI
                origin_country_code: (() => {
                    // 1. Prova dai dati raw dell'API
                    if (apiResponse?.metadata?.raw?.route?.origin?.location?.country_code) {
                        return apiResponse.metadata.raw.route.origin.location.country_code.toUpperCase();
                    }
                    // 2. Deriva dall'aeroporto IATA
                    const originPort = formData.origin || apiResponse?.route?.origin?.port || '-';
                    const countryInfo = getCountryFromAirport(originPort);
                    if (countryInfo.code !== '-') return countryInfo.code;
                    // 3. Fallback con extractCountryCode per compatibilità
                    return extractCountryCode(originPort);
                })(),
                destination_country_code: (() => {
                    // 1. Prova dai dati raw dell'API
                    if (apiResponse?.metadata?.raw?.route?.destination?.location?.country_code) {
                        return apiResponse.metadata.raw.route.destination.location.country_code.toUpperCase();
                    }
                    // 2. Deriva dall'aeroporto IATA
                    const destPort = formData.destination || apiResponse?.route?.destination?.port || '-';
                    const countryInfo = getCountryFromAirport(destPort);
                ...
                // FIX 3: DATE OF DEPARTURE CORRETTA
                date_of_departure: (() => {
                    // 1. Cerca nei movements raw per evento DEP
                    if (apiResponse?.metadata?.raw?.movements && apiResponse.metadata.raw.movements.length > 0) {
                    ...
                    // 3. Cerca nella route origin
                    if (apiResponse?.route?.origin?.date) {
                        return formatDateDDMMYYYY(apiResponse.route.origin.date);
                    }
                    // 4. Cerca nei metadata raw
                    if (apiResponse?.metadata?.raw?.route?.origin?.date_of_departure) {
                        return formatDateDDMMYYYY(apiResponse.metadata.raw.route.origin.date_of_departure);
                    }
                    // 5. Calcola da arrival date - transit time
                    const arrivalDate = apiResponse?.route?.destination?.eta ||
                        apiResponse?.metadata?.raw?.route?.destination?.date_of_rcf;
                    const transitHours = apiResponse?.metadata?.raw?.route?.transit_time;
                    if (arrivalDate && transitHours && typeof transitHours === 'number') {
                        const arrival = new Date(arrivalDate);
                        const departure = new Date(arrival.getTime() - (transitHours * 60 * 60 * 1000));
                        return formatDateDDMMYYYY(departure.toISOString());
                    }
                    // 6. Default: 1 giorno prima della creazione
                ...
                departure: (() => {
                    // Usa la stessa logica di date_of_departure
                    // 1. Cerca nei movements raw per evento DEP
                    if (apiResponse?.metadata?.raw?.movements && apiResponse.metadata.raw.movements.length > 0) {
                        const depMovement = apiResponse.metadata.raw.movements.find(m =>
                            m.event === 'DEP' ||
                ...
                // FIX 4: CONTAINER COUNT (PIECES per AWB)
                container_count: (() => {
                    // 1. Cerca nei cargo pieces
                    ...
                    // 4. Cerca negli eventi per pieces
                    if (apiResponse?.events && apiResponse.events.length > 0) {
                        for (const event of apiResponse.events) {
                            if (event.pieces) {
                                return event.pieces.toString();
                            }
                        }
                    }
                    // 5. Cerca nei movements raw
                    if (apiResponse?.metadata?.raw?.movements && apiResponse.metadata.raw.movements.length > 0) {
                        for (const movement of apiResponse.metadata.raw.movements) {
                            if (movement.pieces) {
                                return movement.pieces.toString();
                            }
                        }
                    }
                    // Default per AWB
                ...
                ...(window.detectedAwbId ? {
                    metadata: {
                        ...
                        raw: apiResponse?.metadata?.raw || {} // Preserva i dati raw
                    }
                } : {
                    metadata: {
                        ...formData.metadata,
                        raw: apiResponse?.metadata?.raw || {} // Preserva i dati raw anche senza ID
                    }
                }),
            } : {}),
