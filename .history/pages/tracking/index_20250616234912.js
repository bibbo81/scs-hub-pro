...
function getColumnFormatter(key) {
    const formatters = {
        ...
        eta: (value, row) => {
            ...
            // 2. Per spedizioni aeree, usa Date Of Arrival come ETA
            if (row.tracking_type === 'awb' && (!etaValue || etaValue === '-')) {
                etaValue = row.metadata?.date_of_arrival || 
                          row.metadata?.['Date Of Arrival'];
            }
            
            // 3. Per spedizioni container/mare, usa Date Of Discharge come ETA  
            if ((row.tracking_type === 'container' || row.tracking_type === 'bl') && (!etaValue || etaValue === '-')) {
                etaValue = row.metadata?.date_of_discharge || 
