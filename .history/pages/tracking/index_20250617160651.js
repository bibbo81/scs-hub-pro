...
// ===== FORMATTERS SHIPSGO COMPLETI - FIX CONTEXT BINDING =====
function getColumnFormatters() {
    // Helper function disponibile in tutte le funzioni
    const getValueHelper = (row, possibleKeys) => {
        // Prova prima nel row diretto
        for (const key of possibleKeys) {
            if (row[key] !== undefined && row[key] !== null && row[key] !== '') {
                return row[key];
    ...
    return {
        ...
        tracking_number: function(value, row) {
            const number = getValueHelper(row, [
                'tracking_number',
                'NUMERO TRACKING', 
                'Numero Tracking',
                'AWB NUMBER',
            ...
            const type = getValueHelper(row, [
                'tracking_type',
                'TIPO',
                'Type'
            ]) || (getValueHelper(row, ['AWB NUMBER', 'AWB Number', 'AIRLINE', 'Airline']) ? 'awb' : 'container');
        ...
        tracking_type: function(value, row) {
            ...
            let type = value || getValueHelper(row, [
                'tracking_type',
                'TIPO',
                'Type'
            ]);
            ...
            if (!type) {
                if (getValueHelper(row, ['AWB NUMBER', 'AWB Number', 'AIRLINE', 'Airline'])) {
                    type = 'awb';
                } else if (getValueHelper(row, ['CONTAINER COUNT', 'Container Count', 'PORT OF LOADING', 'Port Of Loading'])) {
                    type = 'container';
                } else {
        ...
        status: function(value, row) {
            const originalStatus = getValueHelper(row, [
                'status',
                'STATO',
                'Status',
                'STATUS'
        ...
        transit_time: function(value, row) {
            ...
            if (!time) {
                const depDate = getValueHelper(row, ['DATE OF DEPARTURE', 'Date Of Departure']);
                const arrDate = getValueHelper(row, ['DATE OF ARRIVAL', 'Date Of Arrival']);
        ...
        container_count: function(value, row) {
            const count = getValueHelper(row, [
                'container_count',
                'CONTAINER COUNT',
                'Container Count'
            ]) || value || '';
        ...
        co2_emission: function(value, row) {
            const emission = getValueHelper(row, [
                'co2_emission',
                'CO₂ EMISSION (TONS)',
                'CO2 Emission (Tons)'
            ]) || value || '';
        ...
        try {
            ...
            if (stored) {
                ...
                if (trackings.length > 0) {
                    ...
                    // Test con i dati del localStorage
                    const formatters = getColumnFormatters();
                    console.log('\n🧪 FORMATTER TESTS (localStorage data):');
                    console.log('- tracking_type:', formatters.tracking_type(null, trackings[0]));
                    console.log('- status:', formatters.status(null, trackings[0]));
                }
            }
        } catch (e) {
